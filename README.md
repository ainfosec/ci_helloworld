...
`git diff --check` provides a simple way to detect when trailing whitespace
...
useful because some people code with tools that remove trailing whitespace,
which means that if they modify a file in your repo that contains trailing
whitespace, their PRs will contain modifications unrelated to their specific
changes.
...
Finally, Clang Tidy is given it's own make target to simplify its use. Here
...
From Travis CI, we enable Clang Tidy, and dump it's output to a file. If this
...
CppCheck to enable all of it's checks (minus pedantic warnings) and to check
...
tools, you should stick to a specific compile and version.
...
flags to give Valgrind. In this case we enable all of it's checks and tell
